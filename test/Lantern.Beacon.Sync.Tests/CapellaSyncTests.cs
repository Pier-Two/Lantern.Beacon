    ...
    public void AdvanceFinalityWithoutSyncCommittee(string folderPath)
    {
        ...
        foreach (var step in steps)
        {
            if(step.ContainsKey("process_update"))
            {
                ...
                CapellaProcessors.ProcessLightClientUpdate(_syncProtocol._capellaLightClientStore, update, currentSlot, genesisValidatorsRoot, _options, Mock.Of<ILogger<SyncProtocol>>());
    ...
    public void DenebFork(string folderPath)
    {
        ...
        foreach (var step in steps)
        {
            if(step.ContainsKey("process_update"))
            {
                ...
                {
                    var update = CapellaLightClientUpdate.Deserialize(updateData, _options.Preset);
                    DenebProcessors.ProcessLightClientUpdate(_syncProtocol._denebLightClientStore, DenebLightClientUpdate.CreateFromCapella(update), currentSlot, genesisValidatorsRoot, _options, Mock.Of<ILogger<SyncProtocol>>());
                }
            ...
            {
                Assert.That(_syncProtocol._denebLightClientStore.FinalizedHeader.Beacon.GetHashTreeRoot(_options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["upgrade_store"]["checks"]["finalized_header"]["beacon_root"])));
                Assert.That(_syncProtocol._denebLightClientStore.FinalizedHeader.Beacon.Slot, Is.EqualTo(uint.Parse((string)step["upgrade_store"]["checks"]["finalized_header"]["slot"])));
                Assert.That(_syncProtocol._denebLightClientStore.OptimisticHeader.Beacon.GetHashTreeRoot(_options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["upgrade_store"]["checks"]["optimistic_header"]["beacon_root"])));
                Assert.That(_syncProtocol._denebLightClientStore.OptimisticHeader.Beacon.Slot, Is.EqualTo(uint.Parse((string)step["upgrade_store"]["checks"]["optimistic_header"]["slot"])));
                Assert.That(DenebHelpers.GetLcExecutionRoot(_syncProtocol._denebLightClientStore.FinalizedHeader, _options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["upgrade_store"]["checks"]["finalized_header"]["execution_root"])));
                Assert.That(DenebHelpers.GetLcExecutionRoot(_syncProtocol._denebLightClientStore.OptimisticHeader, _options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["upgrade_store"]["checks"]["optimistic_header"]["execution_root"])));
            }
        }
    }
    ...
    public void LightClientSync(string folderPath)
    {
        ...
        foreach (var step in steps)
        {
            if (step.ContainsKey("process_update"))
            {
                ...
                CapellaProcessors.ProcessLightClientUpdate(_syncProtocol._capellaLightClientStore, update, currentSlot,
                    genesisValidatorsRoot, _options, Mock.Of<ILogger<SyncProtocol>>());

                Assert.That(
                    _syncProtocol._capellaLightClientStore.FinalizedHeader.Beacon.GetHashTreeRoot(_options.Preset),
                    Is.EqualTo(TestUtility.HexToByteArray(
                        (string)step["process_update"]["checks"]["finalized_header"]["beacon_root"])));
                Assert.That(_syncProtocol._capellaLightClientStore.FinalizedHeader.Beacon.Slot,
                    Is.EqualTo(uint.Parse((string)step["process_update"]["checks"]["finalized_header"]["slot"])));
                Assert.That(CapellaHelpers.GetLcExecutionRoot(_syncProtocol._capellaLightClientStore.FinalizedHeader, _options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["process_update"]["checks"]["finalized_header"]["execution_root"])));
                Assert.That(
                    _syncProtocol._capellaLightClientStore.OptimisticHeader.Beacon.GetHashTreeRoot(_options.Preset),
                    Is.EqualTo(TestUtility.HexToByteArray(
                        (string)step["process_update"]["checks"]["optimistic_header"]["beacon_root"])));
                Assert.That(_syncProtocol._capellaLightClientStore.OptimisticHeader.Beacon.Slot,
            ...
            else if (step.ContainsKey("force_update"))
            {
                ...
                CapellaProcessors.ProcessLightClientStoreForceUpdate(_syncProtocol._capellaLightClientStore, currentSlot, Mock.Of<ILogger<SyncProtocol>>());
                ...
                Assert.That(CapellaHelpers.GetLcExecutionRoot(_syncProtocol._capellaLightClientStore.FinalizedHeader, _options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["force_update"]["checks"]["finalized_header"]["execution_root"])));
                Assert.That(
                    _syncProtocol._capellaLightClientStore.OptimisticHeader.Beacon.GetHashTreeRoot(_options.Preset),
                    Is.EqualTo(TestUtility.HexToByteArray(
                ...
                Assert.That(CapellaHelpers.GetLcExecutionRoot(_syncProtocol._capellaLightClientStore.OptimisticHeader, _options.Preset), Is.EqualTo(TestUtility.HexToByteArray((string)step["force_update"]["checks"]["optimistic_header"]["execution_root"])));
            }
        }
    }
